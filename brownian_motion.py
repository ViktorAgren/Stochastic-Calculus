...
def get_corr_dW_matrix(
    ...
    """
    2D array of M discrete Brownian Motion increments dW.
    Each column of the array is one process.
    So that the resulting shape of the array is (N, M).
        - N is the number of samples of each process.
...
def get_dW(N: int, random_state: Optional[int] = None) -> np.ndarray:
    """
    Sample N times from a normal distribution,
...
def get_W(N: int, random_state: Optional[int] = None) -> np.ndarray:
    """
    Simulate a Brownian motion discretely samplet at unit time increments.
    Returns the cumulative sum
    """
    dW = get_dW(N, random_state)
